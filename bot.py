...
# ИСПРАВЛЕНИЕ: Функция для запуска polling в фоне
async def run_polling(application: Application):
    """Запускает polling и обрабатывает ошибки"""
    try:
        await application.initialize()
        await application.start()
        await application.updater.start_polling()
        logger.info("✅ PTB бот запущен в режиме polling")
...
async def main():
    ...
    asyncio.create_task(run_polling(ptb_app))
